...
new_df[9:32] <- lapply(new_df[9:32], factor)
...
```
```{r}
#Remove speed_category_1 because it only has 1 unique value and no variation in the predictor value
scaled_df = subset(scaled_df, select = c(-Speed_Category_1))
```
