...
# Import CSV files
library(readr)
subject_info <- read_csv("BME580_Final_Project/subject-info.csv")
test_measure <- read_csv("BME580_Final_Project/test_measure.csv")
```
...
id_merged <- ddply(df_merge, "ID_test", summarise, bmi = mean(bmi), age=mean(Age), sex=mean(Sex), avg_speed=mean(Speed), avg_HR=mean(HR, na.rm=TRUE), avg_VO2=mean(VO2,na.rm=TRUE), avg_VCO2=mean(VCO2, na.rm=TRUE))
...
```{r}
# Create box plots for avg speed, HR, VO2, and VCO2 across gender

# Speed box plot
speed_box <- ggplot(id_merged, aes(x=sex, y=avg_speed)) +
  geom_boxplot() +
  labs(title="Average Speed across Gender", x="Gender", y="Average Speed")

# HR box plot
HR_box <- ggplot(id_merged, aes(x=sex, y=avg_HR)) +
  geom_boxplot() +
  labs(title="Average HR across Gender", x="Gender", y="Average HR")

# V02 box plot
VO2_box <- ggplot(id_merged, aes(x=sex, y=avg_VO2)) +
  geom_boxplot() +
  labs(title="Average VO2 across Gender", x="Gender", y="Average VO2")

# VC02 box plot
VCO2_box <- ggplot(id_merged, aes(x=sex, y=avg_VCO2)) +
  geom_boxplot() +
  labs(title="Average VCO2 across Gender", x="Gender", y="Average VCO2")
...
```{r}
#Female 
female = filter(df_merge, gender == 1)
dim(female)
head(female)

#Male
male = filter(df_merge, gender == 0)
dim(male)
...
matrix
corrplot(matrix, type="upper", tl.col="black", tl.srt=45)
```
Notes, new
predicting per
