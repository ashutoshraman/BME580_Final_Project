...
bw <- 2 * IQR(new_df$Speed_Category) / length(new_df$Speed_Category)^(1/3)

#Histogram with density function of VO2
vo2_hist = ggplot(new_df, aes(x=Speed_Category)) + #Create histogram
  geom_histogram(aes(y=..density..), binwidth=bw)+ #Add density plot
  geom_density() +
  labs(title="Speed Histogram", x="Speed (km/hr)", y="Density")+ 
    theme(
...
new_df = subset(new_df, select = c(-ID_test, -Speed_Category, -avg_VCO2, -avg_VE, -Weight_ind_VO2))
...
#Scale numerical independent variables which everything except for avg_VO2 and, athletic, and Sex and speed categories
scaled_df = new_df
