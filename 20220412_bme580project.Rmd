...
```{r}
cv = createFolds(iris$Species, k = 5)
str(cv)
```
```{r}
accuracies <- c()
index <- 1
for (i in cv) {
  knn_model = knn(train = iris[-i,], 
                 test = iris[i,],
                 cl = iris[-i,]$Species,
                 k = 5)
  CrossTable(iris[i,]$Species, knn_model)
  accuracy = sum(knn_model == iris[i,]$Species)/ length(iris[i,]$Species)
  print(accuracy)
  accuracies[index] = accuracy
  index <- index+1
}
sprintf("Average accuracy across 4 folds is %f", mean(accuracies))

```

```{r}
logModel = glm(outcome ~., family= binomial, data=train)
summary(logModel)
log.fit = predict(logModel, newdata = test, type='response')
head(log.fit, 10)
log.fit = ifelse(log.fit>.5, '1', '0')
head(log.fit,10)
misclassification = sum(log.fit != test$outcome)/ length(test$outcome)
print(misclassification)
print(1-misclassification)

log.fit = predict(logModel, newdata = test, type='response')
pred <- prediction(log.fit, test$outcome)
...
cv = createFolds(heart_data$outcome, k = 4)
str(cv)
...
index <- 1
for (i in cv) {
  knn_model = knn(train = heart_data[-i,], 
                 test = heart_data[i,],
                 cl = heart_data[-i,]$outcome,
                 k = sqrt(length(i)))
  #CrossTable(heart[i,]$outcome, knn_model)
  accuracy = sum(knn_model == heart_data[i,]$outcome)/ length(heart_data[i,]$outcome)
  print(accuracy)
  accuracies[index] = accuracy
  index <- index+1
}
...
Building models for LDA, KNN, LR, and SVM. I will provide justification for each later, but they increase in complexity as you move left to right in model list.

Each model will have K-fold cross validation with 5 folds (80:20 split for train test), because data set is sufficiently large

Within each train fold, I perform gridsearch to tune model hyperparameters. This link shows
how to do so in tidyverse: https://www.r-bloggers.com/2016/12/grid-search-in-the-tidyverse/

This is for manual grid search and quick training, but less control: https://machinelearningmastery.com/tuning-machine-learning-models-using-the-caret-r-package/

This link explains the exact process I'm following: https://stackoverflow.com/questions/42228735/scikit-learn-gridsearchcv-with-multiple-repetitions/42230764#42230764

After all this, I will choose best model parameters from comparing each fold model, and do one more K-fold (80:20), for best model.

I will create classification report of accuracy, precision, recall, F1, sensitivity, specificity, ROC curve. Choose best model from this.
